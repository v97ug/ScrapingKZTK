...
overViewURL = "http://jp.op.gg/ranking/ladder/"

main :: IO ()
main = do
  usersURLMaybe <- scrapeURL overViewURL findUsers
  let usersAndURLs = fromMaybe [(User "", URL "")] usersURLMaybe :: [(User, URL)]
  ...
  let result = (unlines . map show . nub . concat) matchingList
--  putStrLn result
  writeFile "matching.csv" result
